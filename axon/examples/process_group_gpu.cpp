...
Arguments ParseArguments(int argc, char** argv) {
  if (argc < 2 || argc > 4) {
    throw std::runtime_error(
      "usage: process_group_gpu <rank> [store-host] [store-port]");
  }
  return {
...
::c10::intrusive_ptr<::c10d::ProcessGroup> CreateProcessGroup(
  ...
  eux::axon::c10d::InstallAxonBackend(
    process_group, store, std::move(transport),
...
int main(int argc, char** argv) {
  ...
  Check(
    ...
    "rank must be 0 or 1");
  Check(
    at::cuda::device_count() >= kWorldSize,
    "the GPU example requires at least two CUDA devices");
  const ::c10::cuda::CUDAGuard device_guard(arguments.rank);
  const auto device = at::Device(at::kCUDA, arguments.rank);
  auto process_group = CreateProcessGroup(arguments);

  auto value = at::full(
    {1024 * 1024}, static_cast<float>(arguments.rank + 1),
    at::TensorOptions().device(device));
  std::vector<at::Tensor> tensors{value};
  process_group->allreduce(tensors)->wait();
  Check(at::equal(value, at::full_like(value, 3)), "GPU allreduce failed");

  auto message = at::full(
    {1024 * 1024}, -1,
  ...
  if (arguments.rank == 0) {
    ...
    Check(
      at::equal(message, at::full_like(message, 1234)), "GPU receive failed");
  }

  process_group->barrier()->wait();
  std::cout << "Rank " << arguments.rank
            << " completed GPU communication through Axon ProcessGroup.\n";
}
